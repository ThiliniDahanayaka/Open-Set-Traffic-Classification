...
def LoadDataNoDefCW():
    ...
    dataset_dir = "/media/SATA_1/thilini_open_extra/final_codes/Background/DF/dataset/"
    ...
    # Load testing data
    X_open = load(dataset_dir+'X_open.npy')
    X_open = X_open[:, 0:1500]
    y_open = load(dataset_dir+'y_open.npy')

    print ("Data dimensions:")
    print ("X: Training data's shape : ", X_train.shape, X_train.max())
    print ("y: Training data's shape : ", y_train.shape)
    print ("X: Validation data's shape : ", X_valid.shape, X_valid.max())
    print ("y: Validation data's shape : ", y_valid.shape)
    print ("X: Testing data's shape : ", X_test.shape, X_test.max())
