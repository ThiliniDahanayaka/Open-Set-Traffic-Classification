...
def LoadDataIot(trial_num="1", num_classes=8):
    datatpath = '/media/SATA_1/thilini_open_extra/final_datasets/SETA/'

    with open("/media/SATA_1/thilini_open_extra/final_codes/Background/SETA/data_file.json") as config_file:
    ...
    for count1, val in enumerate(openset[0:4]):
        ind = np.where(y==val)[0]
        # print(val, len(ind))
    ...
    for count, val in enumerate(openset[4:]):
        ind = np.where(y==val)[0]
        # print(val, len(ind))
        x_new = np.append(x_new, x[ind][0:20], axis=0)
        # print(x_new.shape)
...
def Load_valid(trial_num="1", num_classes=4):
    datatpath = '/home/sec_user/thilini/Anchor_loss/SETA/dataset/'

    with open("/home/sec_user/thilini/Other_open/background/SETA/data_file.json") as config_file:
        cfg = json.load(config_file)[trial_num]
    closeset = cfg["closed"]
    openset = cfg["open"]
    ...
    for count1, val in enumerate(openset[0:2]):
        ...
        x_new = np.append(x_new, x[ind[0:20]], axis=0)
        # print(x_new.shape)
        y_new = np.append(y_new, np.ones((20,))*num_classes)
