...
for trial in range(1, 6):
	...
	np.save('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/max_positions_'+str(n_max)+'_'+str(trial), res)
...
for trial in range(1, 6):
	...
	max_positions = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/max_positions_'+str(n_max)+'_'+str(trial)+'.npy')
	print('Getting MAVs')
	X, y, _, _, _, _, _, _ = LoadDataNoDefCW(trial_num=str(trial))
	X, y = get_small_set(X, y, 200)
	...
	for c in range(0, n_closed):
		...
		np.save('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(c), logits)
...
for trial in range(1, 6):
	...
	for c in range(0, n_closed):
		...
		for i in idx_c:#idx_c
			...
			a=np.array(mav)
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(c)+'.npy', allow_pickle=True)
			diff = a-b
	...
	np.save('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/threshs_list_'+str(n_max)+'_'+str(trial), threshs)
...
for trial in range(1, 6):
	...
	threshs = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/threshs_list_'+str(n_max)+'_'+str(trial)+'.npy')
	max_positions =  np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/max_positions_'+str(n_max)+'_'+str(trial)+'.npy')
	m_prenult = build(n_closed, model_path, 11)
	probs = []
	...
	for c in range(0, 1):
		...
		for i in range(0, X.shape[0]):
			...
			a=np.array(mav)
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/both/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(int(pred[i]))+'.npy', allow_pickle=True)
			diff = a-b
			diff = np.linalg.norm(diff)
