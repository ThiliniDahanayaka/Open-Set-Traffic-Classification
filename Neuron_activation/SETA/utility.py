...
def LoadDataNoDefCW(n_closed=8, trial_num="1", datasetName='SETA'):
    if datasetName == "SETA":
    ...
    with open("/media/SATA_1/thilini_open_extra/final_datasets/SETA/data_file.json") as config_file:
    ...
    for count, val in enumerate(openset):
        ind = np.where(y==val)[0]
        # print(val, len(ind))
        x_new = np.append(x_new, x[ind][0:14], axis=0)
        # print(x_new.shape)
