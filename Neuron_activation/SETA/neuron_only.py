...
for trial in range(1, 6):
	...
	for c in range(0, n_closed):
		...
		logits = logits[1:]
		np.save('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/neuron_only/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(c), logits)
...
for trial in range(1, 6):
	...
	for c in range(0, n_closed):
		...
		for i in idx_c:#idx_c
			...
			a=np.array(mav[1:])
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/neuron_only/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(c)+'.npy', allow_pickle=True)
			diff = a-b
	...
	np.save('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/neuron_only/threshs_list_'+str(n_max)+'_'+str(trial), threshs)
...
for trial in range(1, 6):
	...
	threshs = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/neuron_only/threshs_list_'+str(n_max)+'_'+str(trial)+'.npy')
	probs = []
	...
	for c in range(0, 1):
		...
		for i in range(0, X.shape[0]):
			...
			a =np.array(mav[1:])
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/SETA/temp/neuron_only/MAVS/_MAV'+str(n_max)+'_'+str(trial)+'_'+str(int(pred[i]))+'.npy')
			diff = a - b
			diff = np.linalg.norm(diff)
			# diff = np.dot(a, b)/( np.linalg.norm(a)* np.linalg.norm(b))
