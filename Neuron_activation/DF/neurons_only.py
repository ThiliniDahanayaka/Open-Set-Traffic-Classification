...
for c in range(0, n_closed):
	...
	logits = logits[1:]
	np.save('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/neuron_only/MAVS/_MAVs_'+str(n_max)+str(c), logits)
...
pred = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/pred_valid.npy')
...
for c in range(0, n_closed):
	...
	for i in idx_c:#idx_c
		...
		a=np.array(mav[1:])
		# b = mavs[int(y[i])]
		b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/neuron_only/MAVS/_MAVs_'+str(n_max)+str(c)+'.npy', allow_pickle=True)
		diff = a-b
...
np.save('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/neuron_only/threshs_list_'+str(n_max), threshs)
...
threshs = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/neuron_only/threshs_list_'+str(n_max)+'.npy')
probs = []
...
for c in range(0, 1):
	...
	for i in range(0, X.shape[0]):
		...
		a =np.array(mav[1:])
		# b = mavs[int(y[i])]
		b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/neuron_only/MAVS/_MAVs_'+str(n_max)+str(int(pred[i]))+'.npy')
		diff = a - b
		diff = np.linalg.norm(diff)
		# diff = np.dot(a, b)/( np.linalg.norm(a)* np.linalg.norm(b))
...
for i in range(0, len(names)):
	# print(i)
	a = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DF/temp/open_'+names[i]+'.npy')
	print(a.shape)
	outs.append(a)
	# outs.append(np.load('/home/sec-user/thilini/open-set_LCN/Tor/layer_out/test_'+names[i]+'.npy'))
