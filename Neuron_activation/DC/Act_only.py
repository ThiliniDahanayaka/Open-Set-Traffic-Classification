...
for trial in range(1, 2):
	...
	for c in range(0, n_closed):
		...
		np.save('/media/SATA_1/thilini_open_extra/final_codes/New/DC/temp/act_only/MAVS/_MAVs_'+str(trial)+'_'+str(n_max)+str(c), logits)
...
for trial in range(1, 2):
	...
	for c in range(0, n_closed):
		...
		for i in idx_c:#idx_c
			...
			a=np.array(mav)
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DC/temp/act_only/MAVS/_MAVs_'+str(trial)+'_'+str(n_max)+str(c)+'.npy', allow_pickle=True)
			diff = a-b
	...
	np.save('/media/SATA_1/thilini_open_extra/final_codes/New/DC/temp/act_only/threshs_list_'+str(trial)+'_'+str(n_max), threshs)
...
for trial in range(1, 2):
	...
	threshs = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DC/temp/act_only/threshs_list_'+str(trial)+'_'+str(n_max)+'.npy')
	m_prenult = build(n_closed, model_path, 11)
	probs = []
	...
	for c in range(0, 1):
		...
		for i in range(0, X.shape[0]):
			...
			a=np.array(mav)
			# b = mavs[int(y[i])]
			b = np.load('/media/SATA_1/thilini_open_extra/final_codes/New/DC/temp/act_only/MAVS/_MAVs_'+str(trial)+'_'+str(n_max)+str(int(pred[i]))+'.npy', allow_pickle=True)
			diff = a-b
			diff = np.linalg.norm(diff)
