...
# def auroc(inData, outData, title='test', in_low =True):
#     # print(inData.shape, outData.shape)
#     inDataMin = np.max(inData, 1)
#     outDataMin = np.max(outData, 1)
#     # print(len(inDataMin), len(outDataMin))
...
#     plt.title(' with AUC:{}'.format(auc))
#     plt.savefig(title+'AWF.png')
#     plt.close()

#     return sklearn.metrics.auc(fpr, tpr)

# def confusion_matrix(pred, label):
#     mat = np.zeros((NB_CLASSES+1, NB_CLASSES+1))
...
# def find_thresh(logits, labels,  num_classes):
...
#     for i, thresh in enumerate(np.linspace(start, end, num=50)):
#         # print('i, thesh = ', i, thresh)
...
#                 temp = np.where(prob>np.linspace(start, end, num=50)[i-1], pred,  num_classes)
#                 temp = np.where(temp==labels, 1, 0)
#                 acc = np.sum(temp)/temp.shape[0]*100
#                 print('thresh: {} acc:{}'.format(np.linspace(start, end, num=50)[i-1], acc))
#                 f_thresh = np.linspace(start, end, num=50)[i-1]
#                 found = True
#             else:
...
#     print('thresh returned: ', f_thresh)
...
_, _, X_valid, y_valid, X_test, y_test = LoadDataNoDefCW()
# Please refer to the dataset format in readme

X_open = np.load("/media/SATA_1/thilini_open_extra/final_datasets/AWF/"+'X_open.npy')
y_open = np.ones((X_open.shape[0],))*NB_CLASSES

# we need a [Length x 1] x n shape as input to the DF CNN (Tensorflow)
X_valid = X_valid[:, :,np.newaxis]
X_test = X_test[:, :,np.newaxis]
X_open = X_open[:, 0:1500,np.newaxis]

print(X_test.shape, X_valid.shape, X_open.shape)
...
# Start evaluating model with testing 
