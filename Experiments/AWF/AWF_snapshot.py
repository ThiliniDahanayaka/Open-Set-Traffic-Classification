...
with open('/home/Yasod/Yasod/AWF/His_Dict', 'wb') as file_pi:
...
gc.collect()
# Please refer to the dataset format in readme
dataset_dir = "/home/Yasod/Yasod/AWF/dataset/"
...
tflite_model_predictions=[]
for i in range(snapshot_number):
  model=load_model('snapshot_'+str(i)+'.hdf5',compile=True)
...
for i in range(snapshot_number):
  Model_pred=[]
  for kk in range(5):
    X_open=np.array(Data['data'][78000*kk:78000*(kk+1)])
    X_open=X_open[:,:1500]
    ...
    Model_pred+=list(PRED)
  Model_pred=np.array(Model_pred)
  print(Model_pred.shape)
  tflite_model_predictions_open.append(Model_pred)
...
y_open = np.array([NB_CLASSES]*len(tflite_model_predictions_open))
