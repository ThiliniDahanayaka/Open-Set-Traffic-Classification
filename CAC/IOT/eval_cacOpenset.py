...
parser.add_argument('--dataset', required = True, type = str, help='Dataset for evaluation', 
									choices = ['IOT', 'SVHN', 'CIFAR10', 'CIFAR+10', 'CIFAR+50', 'CIFARAll', 'TinyImageNet'])
...
# 	auroc = metrics.auroc(inData=xK, outData=xU, title='{}_{}'.format( dataset,name), trial_num=trial_num)

# 	_, valloader, _, mapping = dataHelper.get_train_loaders(dataset, str(trial_num), cfg)
# 	thresh = find_thresh(net, valloader, num_classes, mapping=None)
# 	metrics.classwiseAccuracy(xK, xU, yK, yU,'{}_{}'.format( dataset,name), thresh, trial_num)
# 	metrics.distribution(xK, xU, yK, yU, '{}_{}'.format( dataset,name), trial_num)
# 	all_auroc += [auroc]
