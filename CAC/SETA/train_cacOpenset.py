...
parser.add_argument('--dataset', required = True, type = str, help='Dataset for training', 
									choices = ['SETA', 'CIFAR+50', 'TinyImageNet'])
...
trainloader, valloader, _, mapping = dataHelper.get_train_loaders(dataset, str(trial), cfg)
...
optimizer = optim.Adam(net.parameters(), lr = cfg['openset_training']['learning_rate'][training_iter], 
							weight_decay = cfg['openset_training']['weight_decay'])
